            ...
            <div className="flex flex-wrap justify-center gap-6">
                {news.length > 0 ? (
                    news.map((article, index) => (
                        <div
                            key={index}
                            className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-lg transform transition duration-300 hover:scale-105 flex flex-col w-[220px] relative"
                        >
                            {/* Image Section */}
                            <img
                                style={{ width: "220px", height: "auto" }} // Set image width to 220px and height auto
                                className="object-cover mx-auto mt-4 rounded mb-4"
                                src={article.urlToImage || "https://via.placeholder.com/220"}
                                alt={article.title}
                            />

                            {/* Text Content Section */}
                            <div className="flex flex-col space-y-2 px-4 py-4 mt-4"> {/* Added margin-top to bring text down */}
